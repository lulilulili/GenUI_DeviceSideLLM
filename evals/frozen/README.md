...
- `draft_buckets_v1.jsonl` — 剩余 7 桶的**机器起草候选**（104 条，`status: "needsReview"`）：
  工作流起草+对抗审校产出，已过键名校验与 check_leakage；**未经人工签字，不得用于任何正式评测**。
  用法：每桶挑 ~10 条（可改写）作为你的种子，其余删除，签字后并入正式集。
